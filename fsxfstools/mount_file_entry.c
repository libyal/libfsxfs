...
int mount_file_entry_initialize(
     ...
     size_t name_length,
     libfsxfs_file_entry_t *fsxfs_file_entry,
     libcerror_error_t **error )
{
	...
	*file_entry = memory_allocate_structure(
	...
	( *file_entry )->fsxfs_file_entry = fsxfs_file_entry;
     ...
     libcerror_error_t **error )
{
	...
	if( libfsxfs_file_entry_get_creation_time(
	     file_entry->fsxfs_file_entry,
	     &posix_time,
	     error ) != 1 )
	{
     ...
     libcerror_error_t **error )
{
	...
	/* The XFS file mode matches that of POSIX
	 */
...
int mount_file_entry_get_sub_file_entry_by_index(
     ...
     libcerror_error_t **error )
{
	libfsxfs_file_entry_t *sub_fsxfs_file_entry = NULL;
