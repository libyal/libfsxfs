...
void (*fsxfstools_signal_signal_handler)( fsxfstools_signal_t ) = NULL;

/* Signal handler for Ctrl+C or Ctrl+Break signals
 */
BOOL WINAPI fsxfstools_signal_handler(
             fsxfstools_signal_t signal )
{
	static char *function = "fsxfstools_signal_handler";
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( fsxfstools_signal_signal_handler != NULL )
			{
				fsxfstools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
 ...
 */
void fsxfstools_signal_initialize_memory_debug(
      void )
{
	int flag = 0;
 ...
 */
int fsxfstools_signal_attach(
     void (*signal_handler)( fsxfstools_signal_t ),
     libcerror_error_t **error )
{
	static char *function = "fsxfstools_signal_attach";
	...
	fsxfstools_signal_signal_handler = signal_handler;

	if( SetConsoleCtrlHandler(
	     (PHANDLER_ROUTINE) fsxfstools_signal_handler,
	     TRUE ) == 0 )
	{
		libcerror_error_set(
		 error,
...
#if defined( _MSC_VER )
	fsxfstools_signal_initialize_memory_debug();
#endif
 ...
 * Returns 1 if successful or -1 on error
 */
int fsxfstools_signal_detach(
     libcerror_error_t **error )
{
	static char *function = "fsxfstools_signal_detach";
	...
	fsxfstools_signal_signal_handler = NULL;
