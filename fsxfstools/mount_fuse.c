     ...
     size_t size )
{
	...
	{
		if( libfsxfs_extended_attribute_get_size(
		     extended_attribute,
		     &value_data_size,
		     &error ) != 1 )
		{
     ...
     size_t size )
{
	     ...
	     extended_attribute_index++ )
	{
		...
		if( libfsxfs_extended_attribute_get_utf8_name_size(
		     extended_attribute,
		...
		if( size > 0 )
		{
			...
			if( libfsxfs_extended_attribute_get_utf8_name(
			     extended_attribute,
...
#endif
{
	...
	FSXFSTOOLS_UNREFERENCED_PARAMETER( offset )

#if defined( HAVE_LIBFUSE3 )
	FSXFSTOOLS_UNREFERENCED_PARAMETER( flags )
#endif
...
void mount_fuse_destroy(
      void *private_data FSXFSTOOLS_ATTRIBUTE_UNUSED )
{
	libcerror_error_t *error = NULL;
	static char *function    = "mount_fuse_destroy";

	FSXFSTOOLS_UNREFERENCED_PARAMETER( private_data )
