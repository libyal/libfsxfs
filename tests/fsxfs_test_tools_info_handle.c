...
#include "../fsxfstools/info_handle.h"

/* Tests the info_handle_initialize function
 * Returns 1 if successful or 0 if not
 */
int fsxfs_test_tools_info_handle_initialize(
     void )
{
 ...
 * Returns 1 if successful or 0 if not
 */
int fsxfs_test_tools_info_handle_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	FSXFS_TEST_RUN(
	 ...
	 fsxfs_test_tools_info_handle_initialize );

	FSXFS_TEST_RUN(
	 "info_handle_free",
	 fsxfs_test_tools_info_handle_free );
