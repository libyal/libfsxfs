...
#include "../libfsxfs/libfsxfs_inode_btree_record.h"

uint8_t fsxfs_test_inode_btree_record_data1[ 16 ] = {
	0x00, 0x00, 0x2b, 0x40, 0x00, 0x00, 0x40, 0x32, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xc0, 0x00 };

#if defined( __GNUC__ ) && !defined( LIBFSXFS_DLL_IMPORT )

/* Tests the libfsxfs_inode_btree_record_initialize function
 * Returns 1 if successful or 0 if not
 */
int fsxfs_test_inode_btree_record_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libfsxfs_inode_btree_record_initialize(
	          &inode_btree_record,
	          &error );
	...
	result = libfsxfs_inode_btree_record_free(
	          &inode_btree_record,
	          &error );
	...
	inode_btree_record = (libfsxfs_inode_btree_record_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfsxfs_inode_btree_record_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libfsxfs_inode_btree_record_initialize with memset failing
		 */
...
/* Tests the libfsxfs_inode_btree_record_free function
 * Returns 1 if successful or 0 if not
 */
int fsxfs_test_inode_btree_record_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
/* Tests the libfsxfs_inode_btree_record_read_data function
 * Returns 1 if successful or 0 if not
 */
int fsxfs_test_inode_btree_record_read_data(
     void )
{
	libcerror_error_t *error              = NULL;
	libfsxfs_inode_btree_record_t *inode_btree_record = NULL;
	...
	/* Test regular cases
	 */
	result = libfsxfs_inode_btree_record_read_data(
	          inode_btree_record,
	          fsxfs_test_inode_btree_record_data1,
	          16,
	          &error );
...
#endif
{
	...
	FSXFS_TEST_RUN(
	 "libfsxfs_inode_btree_record_initialize",
	 fsxfs_test_inode_btree_record_initialize );

	FSXFS_TEST_RUN(
	 "libfsxfs_inode_btree_record_free",
	 fsxfs_test_inode_btree_record_free );

	FSXFS_TEST_RUN(
	 "libfsxfs_inode_btree_record_read_data",
	 fsxfs_test_inode_btree_record_read_data );
