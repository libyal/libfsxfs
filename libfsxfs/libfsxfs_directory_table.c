     ...
     libcerror_error_t **error )
{
	...
	else if( directory_table->header->inode_number_data_size == 4 )
	{
		data_offset = 6;
	}
	else
	     ...
	     directory_entry_index++ )
	{
		...
		if( ( io_handle->feature_flags & LIBFSXFS_FEATURE_FLAG_DIRECTORY_V2 ) == 0 )
		{
			name_offset += directory_table->header->inode_number_data_size;
		}
		...
		if( name_offset >= data_size )
		{
			libcerror_error_set(
			 ...
			 LIBCERROR_ERROR_DOMAIN_ARGUMENTS,
			 LIBCERROR_ARGUMENT_ERROR_VALUE_OUT_OF_BOUNDS,
			 "%s: invalid directory entry: %d name offset value out of bounds.",
			 function,
			 directory_entry_index );

			goto on_error;
		}
		name_size = data[ name_offset ];

		entry_data_size += name_size;
